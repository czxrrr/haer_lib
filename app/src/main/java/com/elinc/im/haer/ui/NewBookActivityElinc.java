...
import com.elinc.im.haer.util.PhotoUtil;
...
//    }
    ...
    private void showAvatarPop() {
        ...
        popupWindow.showAtLocation(layout_all_of_new_question, Gravity.BOTTOM, 0, 0);
    }
        ...
        switch (requestCode) {
            case BmobConstants.REQUESTCODE_UPLOADAVATAR_CAMERA:// 拍照修改头像
                if (resultCode == RESULT_OK) {
                    ...
                    startImageAction(Uri.fromFile(file), 300, 400, BmobConstants.REQUESTCODE_UPLOADAVATAR_CROP, true);
                }
                break;
            case BmobConstants.REQUESTCODE_UPLOADAVATAR_LOCATION:// 本地修改头像
                ...
                if (resultCode == RESULT_OK) {
                    ...
                    isFromCamera = false;
                    uri = data.getData();
                    startImageAction(uri, 300, 400, BmobConstants.REQUESTCODE_UPLOADAVATAR_CROP, true);
                } else {
                    ShowToast("照片获取失败");
                }
