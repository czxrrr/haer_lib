...
import com.hasi.haer_lib.ui.MainActivity;
import com.hasi.haer_lib.ui.NewFriendActivity;
...
//			e.printStackTrace();
			...
			BmobLog.i("JsonMsg取得："+e.getMessage());
	...
	public void showOtherNotify(Context context,String username,String toId,String ticker,Class<?> cls){
		...
		if(isAllow && currentUser!=null && currentUser.getObjectId().equals(toId)){
			...
			//同时提醒通知
			BmobNotifyManager.getInstance(context).showNotify(isAllowVoice, isAllowVibrate, R.drawable.ic_launcher, ticker, username, ticker, cls);
		}
	}
	public void showOtherNotify(Context context,String username,String toId,String questionId,String ticker,Class<?> cls){
		boolean isAllow = CustomApplcation.getInstance().getSpUtil().isAllowPushNotify();
		boolean isAllowVoice = CustomApplcation.getInstance().getSpUtil().isAllowVoice();
		boolean isAllowVibrate = CustomApplcation.getInstance().getSpUtil().isAllowVibrate();
		if(isAllow && currentUser!=null && currentUser.getObjectId().equals(toId)){
			...
			//同时提醒通知
			BmobNotifyManager.getInstance(context).showNotifyWithExtras(isAllowVoice, isAllowVibrate, R.drawable.ic_launcher, ticker, username, ticker,intent);
		}
	}
